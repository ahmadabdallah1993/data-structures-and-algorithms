...
const sortByChildren = (charArray) => {
  let a = charArray.sort( (item,item2) => {
    if(item.children.length > item2.children.length){
      return 1;
    } else if(item.children.length < item2.children.length){
      return -1;
    } else return item.house.charCodeAt() - item2.house.charCodeAt();
  });
  return a;
};
